...
namespace quiz.Controllers
{
    public class AuthenticationController : Controller
    {
        ...
        public async Task<IActionResult> GoogleResponse()
        {
            ...
            // You can now use the email to do whatever you need, like checking if the user is registered
            // For example:
            var user = _context.UserQuizzes.FirstOrDefault(u => u.UserQuizEmail == email);
            if (user == null)
            {
                TempData["ErrorMessage"] = "You are not registered. Please sign up first.";
                return RedirectToAction("Register", "Home");
            }

            // Set session variables or whatever you need for your application
            HttpContext.Session.SetString("UserEmail", user.UserQuizEmail);
            HttpContext.Session.SetString("UserRole", user.UserQuizIsAdmin ? "Admin" : "User");
